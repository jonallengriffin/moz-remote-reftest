...
function createMochitestServer(serverBasePath)
{
  ...
  server.registerContentType("list", "text/plain");
  server.setIndexHandler(defaultDirHandler);
  server.identity.setPrimary("http", myHost, SERVER_PORT);
  ...
  do
  {
    ...
    if (options)
    {
      if (options.split(",").indexOf("primary") >= 0)
      {
        ...
        // Primary is now set via a script argument, so just skip
        // server.identity.setPrimary(scheme, myHost, port);
        seenPrimary = true;
        continue;
      }
    }
